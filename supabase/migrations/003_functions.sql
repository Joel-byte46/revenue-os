...
AS $$
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND EXTRACT(DAY FROM NOW() - updated_at) BETWEEN p_min_days AND p_max_days
...
CREATE OR REPLACE FUNCTION get_weighted_pipeline(p_tenant_id UUID, p_days INT DEFAULT 30)
RETURNS JSON
LANGUAGE SQL
SECURITY DEFINER
STABLE
AS $$
  WITH stage_weights AS (
    SELECT unnest(ARRAY['new','qualified','demo_done','proposal_sent','negotiation']) AS stage_name,
  ...
  SELECT json_build_object(
    ...
    COALESCE(SUM(
      CASE WHEN d.close_date <= CURRENT_DATE + p_days
      THEN d.amount * COALESCE(sw.weight,0.1)
      ELSE 0 END
    ),0),

    'deal_count',
    COUNT(*) FILTER (WHERE d.stage NOT IN ('closed_won','closed_lost'))
  )
  FROM deals d
